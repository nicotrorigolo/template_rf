...
Lister cas de tests
    ...
    #FOR    ${excel_feuille_liste}    IN    @{list_sheet}
        ...
        FOR    ${index}    IN RANGE    ${COLONNE_INUTILE}     ${nb_testcases}
            ${choix_dico}    Evaluate    ${index} - ${COLONNE_INUTILE} + ${nb_testcases_total}
            IF  "${ligne_variables_oui_non}[${index}]" == "Individuel_2"
                ${suiteFile} =  Catenate    SEPARATOR=\n\t    ${suiteFile}\n${ligne_variables[${index}]}     ${numero_cas_de_test_en_cours}\n
            END
            ${numero_cas_de_test_en_cours}  Evaluate    ${numero_cas_de_test_en_cours} + 1
        END
    ...
    # ${DICTIONNAIRE_NOM_CAS_DE_TESTS}    BuiltIn.Create Dictionary

    # FOR     ${cellule}  IN RANGE    ${COLONNE_INUTILE}    ${nb_ligne_variables}
    #     ${cellule}    Evaluate    1 + ${cellule}
    #     ${nom_var}   ExcelLibrary.Read Excel Cell    row_num=1     col_num=${cellule}             sheet_name=${EXCEL_FEUILLE}
    #     Collections.Set To Dictionary  ${DICTIONNAIRE_NOM_CAS_DE_TESTS}     ${nom_var}      ${cellule}
    # END

    # Log    ${DICTIONNAIRE_NOM_CAS_DE_TESTS}
